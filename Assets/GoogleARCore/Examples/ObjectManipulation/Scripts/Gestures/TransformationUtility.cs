...
namespace GoogleARCore.Examples.ObjectManipulationInternal
{
    ...
    public static class TransformationUtility
    {
            ...
            TranslationMode translationMode)
        {
                ...
                if (hit.Trackable is DetectedPlane)
                {
                    ...
                    if (IsPlaneTypeAllowed(translationMode, plane.PlaneType))
                    {
                            ...
                            plane.PlaneType == DetectedPlaneType.HorizontalDownwardFacing)
                        {
                            ...
                            groundingPoint = hit.Pose.position;

                            result.PlacementPlane = hit.Trackable;
            ...
            if (translationMode == TranslationMode.Horizontal) {
                Plane groundingPlane =
                    new Plane(Vector3.up, new Vector3(0.0f, UpdateFloorOfTheHouse.floorY, 0.0f));
                ...
                if (groundingPlane.Raycast(cameraRay, out enter))
                {
                    ...
                    groundingPoint = cameraRay.GetPoint(enter);
                    result.HoveringPosition = groundingPoint + (Vector3.up * hoverOffset);

                    result.PlacementPlane = UpdateFloorOfTheHouse.floorDetectedPlane;
                    result.PlacementPosition = groundingPoint;
            ...
            if (translationMode == TranslationMode.Vertical) {
                ...
                float enter;
                float hittedWallEnter = Mathf.Infinity;
                Vector3 hangingPoint = Vector3.zero;

                // Debug.Log("walls " + walls + " wall count " + walls.Count);
                ...
                if (hangingPoint != Vector3.zero) {
                    ...
                    result.PlacementPosition = hangingPoint;
                }
            }
            ...
            if (translationMode == TranslationMode.HorizontalDownWard) {
                ...
                if (groundingPlane.Raycast(cameraRay, out enter))
                {
                    groundingPoint = cameraRay.GetPoint(enter);
                    result.HoveringPosition = groundingPoint;
                    Debug.Log("enter " + groundingPoint);

                    result.PlacementPlane = UpdateFloorOfTheHouse.ceilingDetectedPlane;
                    result.PlacementPosition = groundingPoint;
                    // result.PlacementRotation = Quaternion.identity;
                    return result;
                }
            ...
            TranslationMode translationMode, DetectedPlaneType planeType)
        {
            ...
            if (translationMode == TranslationMode.HorizontalDownWard && planeType == DetectedPlaneType.HorizontalDownwardFacing)
            {
                return true;
            }
        ...
        public struct Placement
        {
            ...
            /// </summary>
            public Trackable PlacementPlane;
